...
def welcome_screen(duration=3):
    ...
    img = p.image.load('C:\\Users\\Naveen knf\\PycharmProjects\\'
     'pythonProject\\DGI\\welcome page.jpg')
    size = img.get_rect().size
    display_start_time = p.time.get_ticks()
    while p.time.get_ticks() - display_start_time <= duration * 1000:
        for event in p.event.get():
            if event.type == p.QUIT:
                p.quit()
                sys.exit()
        s.fill((255, 231, 200))
        s.blit(textsurface, (width // 2 - textsurface.get_width()
         // 2, height // 2 - textsurface.get_height() // 2))
        s.blit(img, (150, 70))
        p.display.flip()
    p.quit()
'''
